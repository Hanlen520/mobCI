...
class Android extends PureComponent {
  ...
  renderStatus = (status) => {
    ...
    switch (status) {
      ...
      case 11:
        ...
        break;
      case 12:
        result = <Badge status="success" text="自动化测试完成"/>
        break;
      case 13:
        result = <Badge status="success" text="正在执行自动化"/>
        break;
      case 14:
        result = <Badge status="success" text="正在安全扫描"/>
        break;
      case 15:
        result = <Badge status="warning" text="安全扫描失败"/>
        break;
      case 16:
        result = <Badge status="success" text="安全扫描完成"/>
        break;
      default:
  ...
  render() {
      ...
      },
      {
        ...
        render: (text, record) => (
          <div className={styles.action}>
            ...
            {record.log_path &&<a href={record.log_path} download="" >查看log</a>}
            {(record.status === 10 || record.status >= 12)&&<a onClick={()=>this.handleShowMoreActionModalVisible(record)}>二维码</a>}
            {(record.status === 12 && record.report_url)&&<Divider type="vertical" />}
            {(record.status === 12 && record.report_url)&&<a href={record.report_url} rel="noopener noreferrer" target="_blank">自动化报告</a>}
            {record.is_perfored === 3 &&<Divider type="vertical" />}
            {record.is_perfored === 3 &&<a href={`/appBuild/android/performance?${record.id}`} rel="noopener noreferrer" target="_blank">性能监控</a>}
    ...
    return (
      <PageHeaderWrapper title="Android打包任务列表">
        <Card bordered={false}>
        ...
        >
          ...
          >
            ...
            <span style={{marginTop:20}}>扫描二维码下载</span>
